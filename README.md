## LLVM Strength Reduction Pass# LLVM Strength Reduction Pass



An educational LLVM function pass showcasing classic strength reductionA custom LLVM optimization pass that performs strength reduction on integer operations.

transformations and custom heuristics for compiler optimization.
...
| Pattern | Replacement | Notes |mkdir build && cd build

|---------|-------------|-------|cmake -DLLVM_DIR=/path/to/llvm/lib/cmake/llvm ..

| `x * 2^k` | `x << k` | Power-of-two multiply |make
...
### Buildingclang -O0 -S -emit-llvm tests/sample.c -o sample.ll

opt -load-pass-plugin=build/libStrengthReducePass.so \

Prerequisites: LLVM (>= 15 recommended) built with shared libraries.    -passes="function(strength-reduce)" \
